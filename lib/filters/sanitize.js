  ...
  try {
    ...
    $("*").each(function () {
      ...
      children.each(function () {
        if (this.type === "text") {
          ...
          const cleaned = text.replace(/\s+/g, " "); // remove trim()
          if (cleaned && cleaned !== " ") {
            $(this).replaceWith(cleaned);
          } else if (cleaned === " ") {
            $(this).replaceWith(" "); // preserve single spaces between inline elements
          } else {
            $(this).remove();
          }
        }
    ...
    $("a[href]").each(function () {
      ...
      if (
        href &&
        href.includes(
          process.env.WP_SITE_URL?.replace(/https?:\/\//, "") ||
            "wordpress.responsible-it.nl",
        )
      ) {
